...
Source was deployed directly through the Vercel connector. Source is now available in the private GitHub repository https://github.com/Simai66/KhuenKan on branch `main` (initial app upload: `3b6a6c7`). Automatic deployment through the Vercel Git integration has not been verified. Environment variables were supplied to the deployment; before any future independent redeploy, ensure the four variables from README are configured in Vercel Project Settings, preserving the encryption key. No secret is included in this archive.
